...
async function listBusyIntervalsForRange(range: TimeRange) {
...
function buildSlotsForWindow(window: TimeRange, busyIntervals: TimeRange[]) {
  const slots: string[] = [];
  ...
  for (
    ...
    cursor + slotDurationMs <= window.endAt.getTime();
...
function validateSlotWithinRules(startAt: Date, endAt: Date) {
  ...
  if (
    startParts.hour < env.CALENDAR_WORKDAY_START_HOUR ||
...
async function ensureSlotIsAvailable(startAt: Date, endAt: Date) {
  ...
  const isUnavailable = busyIntervals.some((busyInterval) =>
    intervalsOverlap({ startAt, endAt }, busyInterval)
  );
