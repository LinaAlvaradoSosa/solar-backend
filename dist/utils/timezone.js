function getFormatter(timeZone, includeWeekday = false) {
...
export function zonedDateTimeToUtc(input, timeZone) {
    let utcTimestamp = Date.UTC(input.year, input.month - 1, input.day, input.hour, input.minute, input.second ?? 0);
    for (let index = 0; index < 3; index += 1) {
        const offsetMinutes = getTimeZoneOffsetMinutes(new Date(utcTimestamp), timeZone);
        const adjustedTimestamp = Date.UTC(input.year, input.month - 1, input.day, input.hour, input.minute, input.second ?? 0) - offsetMinutes * 60_000;
...
export function getLocalWeekday(date) {
...
export function formatInTimeZoneIso(date, timeZone) {
